package com.kingpivot.api.controller.ApiGoodsShopCaseController;

import com.google.common.collect.Maps;
import com.kingpivot.api.dto.goodsShopCase.GoodsShopCaseListDto;
...
@RequestMapping("/api")
@RestController
@Api(description = "商品案例接口")
public class ApiGoodsShopCaseController extends ApiBaseController {

    @Autowired
    private GoodsShopCaseService goodsShopCaseService;

    @ApiOperation(value = "获取商品案例列表", notes = "获取商品案例列表")
    @ApiImplicitParams({
    ...
    @RequestMapping(value = "/getGoodsShopCaseList")
    public MessagePacket getGoodsShopCaseList(HttpServletRequest request) {
        ...
        Page<GoodsShopCase> rs = goodsShopCaseService.list(paramMap, pageable);

        List<GoodsShopCaseListDto> list = null;
        if (rs != null && rs.getSize() != 0) {
            list = BeanMapper.mapList(rs.getContent(), GoodsShopCaseListDto.class);
        ...
        return MessagePacket.newSuccess(rsMap, "getGoodsShopCaseList success!");
    }
}
