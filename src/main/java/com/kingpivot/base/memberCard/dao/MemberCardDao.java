...
public interface MemberCardDao extends BaseDao<MemberCard, String> {
    ...
    @Query(value = "SELECT * FROM memberCard WHERE isValid=1 AND isLock=0 AND memberID=?1 AND NOW() BETWEEN beginTime AND endTime ORDER BY createdTime DESC", nativeQuery = true)
